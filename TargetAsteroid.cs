 ...
 */
public class TargetAsteroid : MonoBehaviour {

    GameObject closest;
    Transform enemyLocation;
    public LineRenderer shootingLine;
    MineAsteroid ShootScript;
    ...
    void Start () {
        ShootScript = this.GetComponent<MineAsteroid>();
        baseRotation = this.transform.localRotation;
        InvokeRepeating("FindClosestEnemy", 2.0f, 2f);
    }
	
	// Update is called once per frame
	void Update () {

        if (closest != null && closest.tag == "Mineable")
        {
            ...
            if (dist < ShootScript.range)
            {
                ...
                if (ShootScript.timer >= ShootScript.timeBetweenBullets)
                {
                    // ... shoot the gun.
                    ShootScript.Shoot();
                   // Debug.Log("shot!");
                ...
                if (ShootScript.timer >= ShootScript.timeBetweenBullets * ShootScript.effectsDisplayTime)
                {
                    // ... disable the effects.
    ...
    {
        GameObject[] gos;
        gos = GameObject.FindGameObjectsWithTag("Mineable");
